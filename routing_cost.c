...
int prepare_objective_dc_cost(struct params_t p, double* alpha, double* obj_coeff) {
...
// prepare constraints
int prepare_constraints_dc_cost(struct params_t p, double* alpha, int* aptrb, int* aptre, int* asub, double* aval) {
...
int prepare_constbounds_dc_cost(struct params_t p, double* alpha, MSKboundkeye* bkc, double* blc, double* buc) {
...
int prepare_varbounds_dc_cost(struct params_t p, MSKboundkeye* bkx, double* blx, double* bux) {
...
int setup_dc_cost(struct params_t p, struct msk_problem_t* dc, double* alpha) {
  ...
  prepare_objective_dc_cost(p, alpha, dc->obj_coeff);
  prepare_constraints_dc_cost(p, alpha, dc->aptrb, dc->aptre, dc->asub, dc->aval);
  prepare_constbounds_dc_cost(p, alpha, dc->bkc, dc->blc, dc->buc);
  prepare_varbounds_dc_cost(p, dc->bkx, dc->blx, dc->bux);
...
int check_avec_cost(int* asub, double* aval, int size) {
  int i;
  for (i=0; i<size; i++) {
    printf("%d %lf\n", asub[i], aval[i]);
  }
...
int optimize_dc_cost(MSKenv_t* env, struct params_t p, struct msk_problem_t* mp, double* beta, MSKrealt* objvalue, int print_sol) {
  ...
  //check_avec_cost(mp->asub, mp->aval, numanz);
