...
int prepare_objective_mn_95_rtaware(struct params_t p, double* beta, double* obj_coeff) {
...
int prepare_constraints_mn_95_rtaware(struct params_t p, double* beta, int* aptrb, int* aptre, int* asub, double* aval) {
  ...
  for (i=0; i<I; i++) {
    for (j=0; j<J; j++) { // for each link -- one constraint
      aptrb[row] = subindex;
      for (c=0; c<C; c++) {
	for (n=0; n<N; n++) {
	  asub[subindex] = i*C*N + c*N + n;
	  aval[subindex] = p.vol[c*N + n] * beta[i*J*C + j*C + c];
	  subindex ++;
	}
      }
      
	// finished constraint for ij'th link
      aptre[row] = subindex;
      row ++;	
    }
  } // finished ij'th link
...
int prepare_constbounds_mn_95_rtaware(struct params_t p, double* beta, MSKboundkeye* bkc, double* blc, double* buc) {
...
// Variable bounds
int prepare_varbounds_mn_95_rtaware(struct params_t p, MSKboundkeye* bkx, double* blx, double* bux) {
...
int setup_mapnode_95_rtaware(struct params_t p, struct msk_problem_t *mp, double* beta) {
  ...
  prepare_objective_mn_95_rtaware(p, beta, mp->obj_coeff);
  prepare_constraints_mn_95_rtaware(p, beta, mp->aptrb, mp->aptre, mp->asub, mp->aval);
  prepare_constbounds_mn_95_rtaware(p, beta, mp->bkc, mp->blc, mp->buc);
  prepare_varbounds_mn_95_rtaware(p, mp->bkx, mp->blx, mp->bux);
...
struct msk_problem_t* alloc_mapnode_95_rtaware(struct params_t p) {
  ...
  // constraint
  int* aptrb = (int*)malloc((I  + I*J + C*N) * sizeof(int)); // number of rows
  int* aptre = (int*)malloc((I  + I*J + C*N) * sizeof(int)); // number of rows
  int* asub  = (int*)malloc((2*I*C*N + I*J*C*N) * sizeof(int)); // number of nonzero terms
  double* aval = (double*)malloc((2*I*C*N + I*J*C*N) * sizeof(double)); // number of nonzero terms

  // cnostraint bounds
  MSKboundkeye* bkc = (MSKboundkeye*)malloc((I+I*J+C*N)*sizeof(MSKboundkeye));
  double* blc = (double*)malloc((I+I*J+C*N)*sizeof(double));
  double* buc = (double*)malloc((I+I*J+C*N)*sizeof(double));
...
int optimize_mapnode_95_rtaware(MSKenv_t* env, struct params_t p, struct msk_problem_t* mp, double* alpha, MSKrealt* objvalue, int print_sol) {
  ...
  int numcon = p.I + p.I * p.J + p.C * p.N;
  int numanz = 2 * p.I * p.C * p.N + p.I * p.J * p.C * p.N;
