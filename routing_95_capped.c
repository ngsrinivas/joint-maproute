...
int prepare_objective_dc_95_lat(struct params_t p, double* alpha, double* obj_coeff) {
...
// prepare constraints
int prepare_constraints_dc_95_lat(struct params_t p, double* alpha, int* aptrb, int* aptre, int* asub, double* aval) {
  ...
  for (i=0; i<I; i++) {
    for (j=0; j<J; j++) { // for each link
      ...
      aptrb[row] = subindex;
      for (c=0; c<C; c++) {
	asub[subindex] = i*J*C + j*C + c; // beta_ijc
	aval[subindex] = 0.0;
	for (n=0; n<N; n++)
	  aval[subindex] += (p.vol[c*N + n] * alpha[i*C*N + c*N + n]);
	subindex++;
...
int prepare_constbounds_dc_95_lat(struct params_t p, double* alpha, MSKboundkeye* bkc, double* blc, double* buc) {
...
int prepare_varbounds_dc_95_lat(struct params_t p, MSKboundkeye* bkx, double* blx, double* bux) {
...
int setup_dc_95_lat(struct params_t p, struct msk_problem_t* dc, double* alpha) {
  ...
  prepare_objective_dc_95_lat(p, alpha, dc->obj_coeff);
  prepare_constraints_dc_95_lat(p, alpha, dc->aptrb, dc->aptre, dc->asub, dc->aval);
  prepare_constbounds_dc_95_lat(p, alpha, dc->bkc, dc->blc, dc->buc);
  prepare_varbounds_dc_95_lat(p, dc->bkx, dc->blx, dc->bux);
...
struct msk_problem_t* alloc_dc_95_lat(struct params_t p) {
  ...
  double* obj_coeff = (double*)malloc((I*J*C)*sizeof(double));
  
  // constraints
  int* aptrb = (int*)malloc((I*J+I*C)*sizeof(int)); // number of rows
  int* aptre = (int*)malloc((I*J+I*C)*sizeof(int)); // number of rows
  int* asub  = (int*)malloc((2*I*J*C) * sizeof(int)); // number of nonzero terms
  double* aval = (double*)malloc((2*I*J*C) * sizeof(double)); // number of nonzero terms

  // bounds on constraints - depends on number of constraints
  MSKboundkeye* bkc = (MSKboundkeye*)malloc((I*J+I*C)*sizeof(MSKboundkeye));
  double* blc = (double*)malloc((I*J+I*C)*sizeof(double));
  double* buc = (double*)malloc((I*J+I*C)*sizeof(double));

  // bounds on variables
  MSKboundkeye* bkx = (MSKboundkeye*)malloc((I*J*C)*sizeof(MSKboundkeye));
  double* blx = (double*)malloc((I*J*C)*sizeof(double));
  double* bux = (double*)malloc((I*J*C)*sizeof(double));
...
int check_avec_95_lat(int* asub, double* aval, int size) {
  int i;
  for (i=0; i<size; i++) {
    printf("%d %lf\n", asub[i], aval[i]);
  }
...
int optimize_dc_95_lat(MSKenv_t* env, struct params_t p, struct msk_problem_t* mp, double* beta, MSKrealt* objvalue, int print_sol) {
  ...
  int numvar = p.I * p.J * p.C;
  int numcon = p.I * p.J + p.I * p.C;
  int numanz = 2 * p.I * p.J * p.C;

  //check_avec_lat(mp->asub, mp->aval, numanz);
