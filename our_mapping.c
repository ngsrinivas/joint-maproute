/*** Functions to prepare objective, coefficients, etc. for the mapping node subproblem ***/
...
int prepare_objective_mn(struct params_t p, double* beta, double* obj_coeff) {
  ...
  for (i=0; i<I; i++) {
    for(c=0; c<C; c++) {
      for(n=0; n<N; n++) { // for every alpha_icn
	coeff_icn = 0.0;
	for(j=0; j<J; j++) {
	  coeff_icn += ((L * p.vol[c*N+n] * p.price[i*J+j] * beta[i*J*C + j*C + c]) + (K * (p.vol[c*N+n]/TT) * p.perf[i*J*C+j*C+c] * beta[i*J*C + j*C + c]));
	}
	obj_coeff[i*C*N + c*N + n] = coeff_icn;
      }
    }
  }

  for (i=0; i<I; i++) {
    for (j=0; j<J; j++) { // for every phi_ij
      if (p.cap[i*J + j] >= 0.000001) {
	obj_coeff[I*C*N + i*J + j] = K * SC_PHI / (p.cap[i*J + j] * p.total_links); // just average over total traffic
      }
      else {
	obj_coeff[I*C*N + i*J + j] = 0.0;
      }
    }
  }
...
int prepare_constraints_mn(struct params_t p, double* beta, int* aptrb, int* aptre, int* asub, double* aval) {
  ...
  for (i=0; i<I; i++) {
	...
	for (c=0; c<C; c++) {
	  for (n=0; n<N; n++) {
	    asub[subindex] = i*C*N + c*N + n;
	    aval[subindex] = y_coeff[k] * p.vol[c*N + n] * beta[i*J*C + j*C + c];
	    subindex ++;
	  }
	}
	
	asub[subindex] = I*C*N + i*J + j;
	aval[subindex] = -1.0; // for phi_ij in the constraint
	subindex ++;

	// finished k'th constraint for ij'th link
	aptre[row] = subindex;
	row ++;	
      } // finished k'th constraint for ij'th link
  ...
  // set 3. Traffic inclusion from mapping nodes
  for (c=0; c<C; c++) {
...
int prepare_constbounds_mn(struct params_t p, double* beta, MSKboundkeye* bkc, double* blc, double* buc) {
...
// Variable bounds
int prepare_varbounds_mn(struct params_t p, MSKboundkeye* bkx, double* blx, double* bux) {
  ...
  for (i=0; i<I; i++) {
    for (c=0; c<C; c++) {
      for (n=0; n<N; n++) {
	bkx[var] = MSK_BK_LO;
	// change lower bounds for checking marginal cost arguments
	//blx[var] = 0.0; 
...
int setup_mapnode(struct params_t p, struct msk_problem_t *mp, double* beta) {
  ...
  prepare_objective_mn(p, beta, mp->obj_coeff);
  prepare_constraints_mn(p, beta, mp->aptrb, mp->aptre, mp->asub, mp->aval);
  prepare_constbounds_mn(p, beta, mp->bkc, mp->blc, mp->buc);
  prepare_varbounds_mn(p, mp->bkx, mp->blx, mp->bux);
...
struct msk_problem_t* alloc_mapnode(struct params_t p) {
  ...
  double* obj_coeff = (double*)malloc((I*C*N + I*J)*sizeof(double));

  // constraint
  int* aptrb = (int*)malloc((I  + 6*I*J + C*N) * sizeof(int)); // number of rows
  int* aptre = (int*)malloc((I  + 6*I*J + C*N) * sizeof(int)); // number of rows
  int* asub  = (int*)malloc((2*I*C*N + 6*I*J*C*N + 6*I*J) * sizeof(int)); // number of nonzero terms
  double* aval = (double*)malloc((2*I*C*N + 6*I*J*C*N + 6*I*J) * sizeof(double)); // number of nonzero terms

  // cnostraint bounds
  MSKboundkeye* bkc = (MSKboundkeye*)malloc((I+6*I*J+C*N)*sizeof(MSKboundkeye));
  double* blc = (double*)malloc((I+6*I*J+C*N)*sizeof(double));
  double* buc = (double*)malloc((I+6*I*J+C*N)*sizeof(double));

  // variable bounds
  MSKboundkeye* bkx = (MSKboundkeye*)malloc((I*C*N + I*J)*sizeof(MSKboundkeye));
  double* blx = (double*)malloc((I*C*N + I*J)*sizeof(double));
  double* bux = (double*)malloc((I*C*N + I*J)*sizeof(double));
...
int optimize_mapnode(MSKenv_t* env, struct params_t p, struct msk_problem_t* mp, double* alpha, MSKrealt* objvalue, int print_sol) {
  ...
  int numvar = p.I * p.C * p.N + p.I * p.J;
  int numcon = p.I + 6 * p.I * p.J + p.C * p.N;
  int numanz = 2 * p.I * p.C * p.N + 6 * p.I * p.J * p.C * p.N + 6 * p.I * p.J;
...
// Just split traffic equally -- ie, initialized to round robin mode.
int init_alpha(struct params_t p, double* alpha) {
  ...
  /* int* chosen_dc = (int*)malloc(C*N*sizeof(int)); */

  /* // make a random choice for the "major" DC for each client and mapnode. */
  /* for (c=0; c<C; c++) { */
  /*   for (n=0; n<N; n++) { */
  /*     chosen_dc[c*N+n] = rand() % I; */
  /*   } */
  /* } */

  /* // apply choice to each c,n. */
  /* // if chosen_dc[cn] is i, "select" it by allotting majority of traffic there. */
  /* // else, allot infinitesimal traffic there. */
  /* for (i=0; i<I; i++) { */
  /*   for (c=0; c<C; c++) { */
  /*     for (n=0; n<N; n++) { */
  /* 	alpha[i*C*N + c*N + n] = MY_EPSILON * (chosen_dc[c*N + n] != i) + (1 - MY_EPSILON * (I-1)) * (chosen_dc[c*N + n] == i); */
  /*     } */
  /*   } */
  /* } */
