...
int prepare_objective_mn_95_capped_closest(struct params_t p, double* obj_coeff) {
...
int prepare_constraints_mn_95_capped_closest(struct params_t p, int* aptrb, int* aptre, int* asub, double* aval) {
...
int prepare_constbounds_mn_95_capped_closest(struct params_t p, MSKboundkeye* bkc, double* blc, double* buc) {
  ...
  for (i=0; i<I; i++) {
    ...
    double tcap = 0.0;
    for (j=0; j<J; j++) tcap += (CAPFRAC2 * p.cap[i*J + j]);
    bkc[row] = MSK_BK_UP;
    blc[row] = 0.0;
    buc[row] = tcap;
    row ++;
...
// Variable bounds
int prepare_varbounds_mn_95_capped_closest(struct params_t p, MSKboundkeye* bkx, double* blx, double* bux) {
...
int setup_mapnode_95_capped_closest(struct params_t p, struct msk_problem_t *mp) {
  ...
  prepare_objective_mn_95_capped_closest(p, mp->obj_coeff);
  prepare_constraints_mn_95_capped_closest(p, mp->aptrb, mp->aptre, mp->asub, mp->aval);
  prepare_constbounds_mn_95_capped_closest(p, mp->bkc, mp->blc, mp->buc);
  prepare_varbounds_mn_95_capped_closest(p, mp->bkx, mp->blx, mp->bux);
